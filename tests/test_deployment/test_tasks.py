...
def pytest_generate_tests(metafunc):
    if (
        "os_type" in metafunc.fixturenames
        and "combadge_version" in metafunc.fixturenames
    ):
        metafunc.parametrize(
            ...
            [
                ...
                if os_type
                not in UNSUPPORTED_OS_TYPES_PER_COMBADGE_VERSION[combadge_version]
            ],
        )
...
def test_exec_ssh_command(os_type):
    with get_remote_host(os_type) as remote_host:
        assert standardize_hostname(
            remote_host.exec_ssh_command("hostname")
        ) == standardize_hostname(remote_host._host)
